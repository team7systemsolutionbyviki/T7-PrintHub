...
export const DBService = {
  ...
  // ══════════════════════════════════════════════════════════════════════
  //  STATIONERY & SALES PRODUCTS (Pens, Pencils, Accessories CRUD)
  // ══════════════════════════════════════════════════════════════════════
  ...
  async getProductsCatalog() {
    ...
    if (!isDemo && db) {
      try {
        ...
        console.warn('[PRODUCTS] Firestore fetch error:', e);
      }
    }
    // Seed with defaults
    _productsCache = DEFAULT_PRODUCTS.map(p => ({ category: 'Stationery', status: 'Active', stockStatus: 'In Stock', ...p }));
    if (!isDemo && db) {
      (async () => {
        try {
          const { doc, setDoc } = await fs();
          for (const item of _productsCache) {
            if (item.id) await setDoc(doc(db, 'products', item.id), item);
          }
        } catch (e) {}
      })();
    }
  ...
  async saveProductItem(productData) {
    ...
    if (!isDemo && db) {
      try {
        const { doc, setDoc } = await fs();
        await setDoc(doc(db, 'products', targetItem.id), targetItem);
      } catch (e) {
